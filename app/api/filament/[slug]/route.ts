...
export async function GET(
    ...
    try {
        ...
        // Fetch by slug for SEO-friendly URLs, or by ID if slug lookup fails
        ...
        // Get related colors from FilamentColorGroup
        // This includes the current filament and other colors in the same group
        const groupCategory = `${filament.material} ${filament.finishType}`;
        
        const colorGroupEntries = await prisma.filamentColorGroup.findMany({
            where: {
        ...
        // Sort: current filament first, then others by displayOrder
        const allRelatedFilaments = colorGroupEntries.map(entry => entry.filament);
        const relatedFilaments = [
            ...allRelatedFilaments.filter(f => f.id === filament.id), // Current first
            ...allRelatedFilaments.filter(f => f.id !== filament.id), // Others after
        ];
        ...
        const transformedFilament = {
            ...
            // Related colors
            colours: relatedFilaments.map(c => ({
                id: c.id,
                name: c.colorName,
                hex: c.hexCode,
                slug: c.slug,
