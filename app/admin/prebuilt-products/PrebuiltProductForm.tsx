...
import { Loader2, Plus, X, ArrowUp, ArrowDown, Star } from "lucide-react";
...
export default function PrebuiltProductForm({
      ...
      setFormData({
        ...
        sizes:
          product.sizes?.map((size) => ({
            ...
            sizeType: size.sizeType as "standard" | "fractional" | "custom",
          })) || [],
      });
    } else {
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.price <= 0 || formData.originalPrice <= 0) {
      toast({
        title: "Error",
        description: "Product Price and Original Price must be greater than 0.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch("/api/prebuilt-products/upload-image", {
        method: "POST",
        body: uploadFormData,
      });
      ...
      <DialogContent className="max-w-[90vw] w-[1200px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-3xl font-bold mb-2">
            {product ? "Edit Prebuilt Product" : "Add New Prebuilt Product"}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-4">
                <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-4">
                  ...
                  <p className="text-sm text-muted-foreground mt-2">
                    Upload product images (drag and drop supported)
                  </p>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateSize(index, "originalPrice", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <select
                      value={size.sizeType}
                      onChange={(e) =>
                        updateSize(index, "sizeType", e.target.value)
                      }
          ...
          {/* Product Settings */}
          <div className="space-y-6 bg-muted/30 p-6 rounded-lg">
            <h3 className="text-xl font-semibold mb-4">Product Settings</h3>
            <div className="space-y-4">
