  ...
  const handlePayment = async () => {
    try {
      ...
      const transactionId = `T${Date.now()}${Math.random()
        .toString(36)
        .slice(2)}`;
      console.log("[Payment Flow] Generated transaction ID:", transactionId);
  ...
  const checkPaymentStatus = useCallback(
    async (transactionId: string, orderId: string) => {
      try {
        ...
        console.log("[Payment Status] Payment status response:", response.data);
        ...
        if (response.data.success && response.data.code === "PAYMENT_SUCCESS") {
          ...
          const storedAmount = sessionStorage.getItem("phonepe_amount");
          ...
          setTimeout(() => checkPaymentStatus(transactionId, orderId), 5000);
        } else {
        ...
        toast({
          title: "Error",
          description: "An error occurred while checking your payment status.",
          variant: "destructive",
        });
